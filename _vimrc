...
" * Search & Replace
" * Spelling
" * Keystrokes -- Make Keybindings Work when Typing in Russian
...
if &term =~ 'xterm'
  ...
  if $COLORTERM == 'gnome-terminal'
    ...
    " enable advanced mouse handling (dragging windows status to resize
    " splits, etc)
    set ttymouse=xterm2
  ...
  " don't:
  elseif $COLORTERM == ''
    " for me the next line results in BkSpace behaving prety much like <Del>, I
    " don't like it!
    "execute 'set t_kb=' . nr2char(8)
    fixdel
...
" do not ring bell on error, do not flash screen
set visualbell
set t_vb=
...
" remembenfo for 10 files, but never any on removable disks, don't remember
" marks in file don't rehighlight old search patterns, and only save up to
...
" allow reading .vimrc files in the current directory
set exrc

" but deny executing shell commands and writing files
set secure

" automaticly write changes when calling :make, jumping to another
" file via tags, etc.
set autowrite

" do not create backups when writing files
set nobackup

" always display status line (displayed anyway if a window is split)
set laststatus=2

" more information in status
set statusline=%<%f%=\ [%1*%M%*%n%R%H]\ %9(%l\/%L%)\ %-5(%c%V%)\ %3p%%

" higlight inconsistent and extraneous whitespace as errors; do not
" treat some of gcc extensions to C as errors
let c_space_errors=1
let c_no_curly_error=1

" setup a more convenient split direction 
set splitbelow
set splitright
...
" wrap and put visual indication
set wrap
set showbreak=~~~
...
" to re-enable tabs, execute the following
"set shiftwidth=8
"set tabstop=8
"set noexpandtab
...
" do not offset case labels when c-indenting; fix indenting when in
